...
window.onload = (event) => {
	...
	setTimeout(function () {
		...
		snowEffect = snowFall.snow(document.getElementsByTagName('body')[0], { image: effectSetting.icon, minSize: effectSetting.minSize, maxSize: effectSetting.maxSize, flakeCount: effectSetting.number, maxSpeed: effectSetting.speed, minSpeed: minSpeed });
		// }
	}, 300);
...
function clearSnow(container) {
    // Lấy tất cả các bông tuyết
    var flakes = container.getElementsByClassName("snowfall-flakes");
    
    // Xóa từng bông tuyết
    while (flakes.length > 0) {
        flakes[0].parentNode.removeChild(flakes[0]);
    }
}

function reloadFall() {
	console.log('ReloadFall Active!!!')
	let effectSetting = biicore.effect.setting;
	let minSpeed = (parseInt(effectSetting.speed) - 3);
	if (minSpeed <= 0) minSpeed = 1;
	clearSnow(document.body);
